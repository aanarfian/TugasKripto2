...
class NTRUencrypt:
    ...
    def readPub(self,pub):
        """
        Read a public key file, generate a new r value based on new N
        """
        self.p  = int(pub.splitlines()[0].split(" ")[0])
        self.q  = int(pub.splitlines()[0].split(" ")[1])
        self.N  = int(pub.splitlines()[0].split(" ")[2])
        self.dr = int(pub.splitlines()[0].split(" ")[3])
        self.h  = np.array(pub.splitlines()[1].split(' '),dtype=int)
        self.I         = np.zeros((self.N+1,), dtype=int)
        self.I[self.N] = -1
        self.I[0]      = 1
        self.genr()
