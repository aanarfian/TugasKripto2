...
class NTRUdecrypt:
    ...
    def writePub(self,filename="key"):
        ...
        """
        pubHead = str(self.p) + ' ' + str(self.q) + ' ' + str(self.N) \
             + ' ' + str(self.dr) + '\n' + ' '.join(map(str,self.h))
        return(pubHead)
    ...
    def writePriv(self,filename="key"):
        ...
        """
        privHead = str(self.p) + ' ' + str(self.q) \
            + ' ' + str(self.N) + ' ' + str(self.df) + ' ' + str(self.dg) \
            + ' ' + str(self.dr) + '\n' + str(' '.join(map(str,self.f))) + '\n' + ' '.join(map(str,self.fp)) + '\n' + ' '.join(map(str,self.fq)) + '\n' + ' '.join(map(str,self.g))
        return(privHead)


    def readPriv(self,priv):
        """
        Read a public key file
        """
        self.p  = int(priv.splitlines()[0].split(" ")[0])
        self.q  = int(priv.splitlines()[0].split(" ")[1])
        self.N  = int(priv.splitlines()[0].split(" ")[2])
        self.df = int(priv.splitlines()[0].split(" ")[3])
        self.dg = int(priv.splitlines()[0].split(" ")[4])
        self.dr = int(priv.splitlines()[0].split(" ")[5])
        self.f  = np.array(priv.splitlines()[1].split(" "),dtype=int)
        self.fp = np.array(priv.splitlines()[2].split(" "),dtype=int)
        self.fq = np.array(priv.splitlines()[3].split(" "),dtype=int)
        self.g  = np.array(priv.splitlines()[4].split(" "),dtype=int)
        self.I         = np.zeros((self.N+1,), dtype=int)
        self.I[self.N] = -1
        self.I[0]      = 1
    ...
    def genPubPriv(self,keyfileName="key"):
        ...
        self.genfg()
        self.genh()
        pub = self.writePub(keyfileName)
        priv = self.writePriv(keyfileName)
        return (pub,priv)
